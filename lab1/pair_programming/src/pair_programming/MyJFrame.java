		...
		queryItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (ta.getText().length() == 0) {
					...
					JOptionPane.showMessageDialog(null, querybRIDGEwORDS(G, word1, word2), "结果",
			...
			public void actionPerformed(ActionEvent e) {
				if (ta.getText().length() == 0) {
					...
					try {
						...
						JOptionPane.showMessageDialog(null, generateNewText(G, inputtext), "结果",
								JOptionPane.INFORMATION_MESSAGE);
					} catch (Exception e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
