...
class myprint extends JPanel{
	...
	public void paintcomponents(Graphics g,Graph G) throws IOException{
	    ...
	    gv.addln("A -> B[color=red];");
	    gv.addln("A -> C;");*/
	    gv.addln(gv.end_graph());
	/*    System.out.println(gv.getDotSource());
	    File out = new File("C:\\Users\\123\\Desktop\\out." + "jpg");    // Windows
	...
	public void highlight(int[] a,Graphics g,Graph G) throws IOException{
	    ...
	    for (int i=0;i<G.count();i++){
	    	...
	    	for (int j=0;j<G.count();j++){
	    		if(G.adjacency_matrix()[i][j]>0){
	    			gv.addln(str+" -> "+G.point_name()[j]+"[ color = "+color_set[ag[i][j]]+", label = "+G.adjacency_matrix()[i][j]+" ];");
	    		}
	    	}
	    }
	...
	public BufferedImage Screenshot(int x,int y){
		try{
			BufferedImage image = new Robot().createScreenCapture(new  
					Rectangle(x+5, y+66, (int) this.getWidth()-10, (int) this.getHeight()));  
			return image;
		}
		catch (Exception e) {
			e.printStackTrace();
